  ...
  io.sockets.on('connection', function (client) {
    ...
    client.on('connectToIRC', function (data) {
      ...
      var opts = {
        ...
        channels:     data.options.channels.replace(/\s+/g,'').split(","),
      ...
      ircClient.addListener('abort', function (message) {
        client.emit('abort', message);
      });

      ircClient.addListener('connectionFailure', function (message) {
        client.emit('connectionFailure', message);
      });
